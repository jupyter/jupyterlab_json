...
if [ $? -eq 0 ]; then
    ...
    a developer install of jupyterlab_json you must have pip installed on
...
if [ $? -eq 0 ]; then
    ...
    if [[ "$OSTYPE" == "msys" ]]; then
        jupyter labextension install --py $nbExtFlags jupyterlab_json
    else
        jupyter labextension install --py --symlink $nbExtFlags jupyterlab_json
    fi
    jupyter labextension enable --py $nbExtFlags jupyterlab_json
else
    echo "'jupyter lab --version' failed, therefore jupyter lab is not installed. In
    order to perform a developer install of jupyterlab_json you must
...
if [ $? -eq 0 ]; then
    ...
    if [[ "$OSTYPE" == "msys" ]]; then
        jupyter nbextension install --py $nbExtFlags jupyterlab_json
    else
        jupyter nbextension install --py --symlink $nbExtFlags jupyterlab_json
    fi
    jupyter nbextension enable --py $nbExtFlags jupyterlab_json
else
    ...
    jupyterlab_json you must have jupyter notebook on your machine!
